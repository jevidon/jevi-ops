...
export function TaskStatusControl({ task, children }: { task: StatusTask; children?: ReactNode }) {
  ...
  const [result, setResult] = useState<{ error?: string; detailsHref?: string; reload?: boolean }>({});
  ...
  return <div className="shrink-0 max-w-[180px]">
    <select aria-label="Task status" value={selected?.id ?? ''} disabled={pending || result.reload}
      ...
      onChange={e => {
        ...
        startTransition(async () => {
          setResult({});
          try {
            setResult(await changeWorkflowStatus(task.id, id, config.revision));
          } catch (error) {
            // This rejection happens before the server action runs, so its
            // own error handler cannot catch it. Keep the page usable and
            // fetch fresh client code before the user tries the change again.
            if (unstable_isUnrecognizedActionError(error)) {
              setResult({ error: 'The app has updated. Reload the page, then try changing the status again.', reload: true });
              return;
            }
            setResult({ error: 'Could not save the status. Please try again.' });
          }
        });
      }}>
    ...
    {result.error && <p role="alert" className="mt-1 text-[12px] text-accent">
      {result.error}{' '}
      {result.detailsHref && <Link href={result.detailsHref} className="underline">Enter details</Link>}
      {result.reload && <button type="button" className="underline" onClick={() => window.location.reload()}>Reload page</button>}
    </p>}
  </div>;
}
